...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        {{-- Header --}}
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-slate-900">Buat Lowongan Baru</h1>
            <p class="text-sm text-slate-600 mt-1">Isi formulir di bawah untuk membuat lowongan pekerjaan</p>
        </div>

        {{-- Form Card --}}
        <div class="bg-white rounded-xl shadow-sm border border-slate-200">
            <form action="{{ route('perusahaan.lowongan.store') }}" method="POST" class="p-6 space-y-6" id="lowonganForm">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="tipe_pekerjaan"
                            ...
                            <option value="full-time" {{ old('tipe_pekerjaan') == 'full-time' ? 'selected' : '' }}>Full Time
                            </option>
                            <option value="part-time" {{ old('tipe_pekerjaan') == 'part-time' ? 'selected' : '' }}>Part Time
                            </option>
                            <option value="kontrak" {{ old('tipe_pekerjaan') == 'kontrak' ? 'selected' : '' }}>Kontrak
                            </option>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <input type="number" name="gaji_min" value="{{ old('gaji_min') }}" min="0"
                        ...
                        <div>
                            <input type="number" name="gaji_max" value="{{ old('gaji_max') }}" min="0"
                ...
                <div>
                    ...
                    <textarea name="deskripsi_pekerjaan" rows="5"
                        ...
                        placeholder="Jelaskan deskripsi pekerjaan, tanggung jawab, dan detail lainnya...">{{ old('deskripsi_pekerjaan') }}</textarea>
                ...
                <div>
                    ...
                    <textarea name="kualifikasi" rows="5"
                        ...
                        placeholder="Tulis kualifikasi yang dibutuhkan, skill, pengalaman, dll...">{{ old('kualifikasi') }}</textarea>
                    @error('kualifikasi')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
    ...
    @push('scripts')
        <script>
            document.getElementById('lowonganForm').addEventListener('submit', function(e) {
                ...
                submitBtn.innerHTML = '<i class="ri-loader-4-line mr-2 animate-spin"></i> Menyimpan...';
